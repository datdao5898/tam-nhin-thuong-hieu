<section
    class="relative py-20 bg-gradient-to-b from-[#0B022D] to-[#05021A] bg-cover text-white mx-auto justify-items-center
"
    style="background-image: url('/assets/bg-about.png');"
>
    <div class="container">
        <img
            src="/assets/tnths.webp"
            alt="Về Ban Đối Ngoại"
            class="w-100 mb-12"
        />
    </div>
    <div
        class="container mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-12 items-center mb-12"
    >
        <div class="flex flex-col items-center gap-10">
            <img
                src="/assets/doitac.jpg"
                alt="Ban Đối Ngoại"
                class="rounded-[35px] border border-white"
            />
            <h2 class="text-4xl">ĐỐI TÁC</h2>
        </div>

        <div class="flex flex-col items-center gap-10">
            <img
                src="/assets/bgk.webp"
                alt="Ban Đối Ngoại"
                class="rounded-[35px] border border-white"
            />
            <h2 class="text-4xl">BAN GIÁM KHẢO</h2>
        </div>
        <div class="flex flex-col items-center gap-10">
            <img
                src="/assets/thisinh.jpg"
                alt="Ban Đối Ngoại"
                class="rounded-[35px] border border-white"
            />
            <h2 class="text-4xl">THÍ SINH</h2>
        </div>
    </div>
</section>



